...
;; Version: 0.0.1
;; Package-Requires: helm, json, url, request
...
alist of headers, and DATA is request body data as JSON."
  (request-response-data
   (request url
            :type method :params q-params
            :parser (if parse-json 'json-read 'buffer-string)
...
(defun spot4e-helm-alist-chain ()
  (let* ((q-params (if (not spot4e-dynamic-helm) spot4e-q-params
                     (append spot4e-q-params `(("q" . ,helm-pattern)))))
         (resp (spot4e-request "GET" spot4e-url q-params t)))
    (alist-get-chain spot4e-alist-address resp)))
  ...
  (let ((url (cond ((equal type "search")
    ...
    (spot4e-helm "spot4e-track-candidates"
                 ...
                 (when (equal type "search")
                   t)
                 (append `(("access_token" . ,spot4e-access-token)
                           ("limit" . "50"))
                         extra-q-params)
...
(defun spot4e-helm-search-artists ()
  ...
  (spot4e-helm-artists "search" `(("type" . "artist"))))
...
(defun spot4e-helm-search-user-artists ()
  ...
  (spot4e-helm-artists "user" `(("type" . "artist"))))
...
buffer and is ignored."
  ...
  (spot4e-helm-albums "search" `(("type" . "album"))))
...
(defun spot4e-helm-search-artist-albums (selection)
  ...
  (spot4e-helm-albums "artist" `(("album_type" . "album,single")) selection))
...
(defun spot4e-helm-search-tracks ()
  ...
  (interactive)
  (spot4e-helm-tracks "search" `(("type" . "track"))))
...
type of track object is given by TYPE."
  ...
  (let ((track-id (if selection
    ...
    (message track-id)
    (spot4e-helm-tracks "rec" `(("seed_tracks" . track-id)
                                ("access_token" . spot4e-access-token)
                                ("limit" . "50")))))
